...
class MMEhoiNetv1(EhoiNet):
    ...
    def inference(self, batched_inputs: List[Dict[str, torch.Tensor]], detected_instances: Optional[List[Instances]] = None, do_postprocess: bool = True):
        ...
        if self.classification_gloves is not None:
            tmp_time = time.time()
            glove_logits = self.classification_gloves(self._c_hands_features)
            output_classification_gloves = torch.round(torch.sigmoid(glove_logits)).int()
        ...
        if len(instances_hands_updated) > 0:
            additional_outputs = Instances(instances_hands_updated.image_size)
            additional_outputs.set("pred_boxes", instances_hands_updated.pred_boxes)
            ...
            if instances_hands_updated.has("contact_states"):
                additional_outputs.set("contact_states", instances_hands_updated.contact_states)
            if instances_hands_updated.has("gloves"):
                additional_outputs.set("gloves", instances_hands_updated.gloves)
            if instances_hands_updated.has("dxdymagn_hand"):
                additional_outputs.set("dxdymagn_hand", instances_hands_updated.dxdymagn_hand)
            if instances_hands_updated.has("pred_keypoints"):
                additional_outputs.set("pred_keypoints", instances_hands_updated.pred_keypoints)
