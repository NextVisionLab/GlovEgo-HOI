...
def parse_args():
    ...
    if args.test_json is None:
        args.test_json = []
    if args.test_dataset_names is None:
...
def load_cfg(args, num_classes):
    ...
    cfg.MODEL.MASK_ON = args.mask_gt
...
def main():
    ...
    with EventStorage(start_iter) as storage:
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if comm.is_main_process():
                if not args.no_wandb:
                    wandb.log({"total_loss": losses_reduced, **loss_dict_reduced, "lr": storage.latest()["lr"][0]}, step=iteration + 1)
                ...
                elif (iteration + 1) % 20 == 0 or iteration == max_iter:
                    for writer in writers:
                        writer.write()
